...
describe("add paths survive a JSON reload", () => {
  it("keeps parent, partner, child, sibling, and unlinked people", () => {
    ...
    assert.equal(restored.people.length, tree.people.length);
    assert.equal(restored.unions.length, tree.unions.length);
    assert.equal(restored.childLinks.length, tree.childLinks.length);
  });
});
