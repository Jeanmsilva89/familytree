...
export function buildGraph(tree: TreeData, focusHint?: string): GraphLayout {
  ...
  const primaryIds = primary ? primary.partnerIds.filter((id) => personById(tree, id)) : [focus.id];
  ...
  const parentUnion = tree.unions.find(
    (u) => focusParents.length > 0 && focusParents.every((id) => u.partnerIds.includes(id)),
  );
  ...
  const coupleXs = orderedPrimary.map((id) => places.get(id)?.x).filter((x): x is number => Number.isFinite(x));
  const coupleRight = coupleXs.length ? Math.max(...coupleXs) : (places.get(focus.id)?.x ?? 0);
  let cursor = coupleRight + CARD.w + CARD.gap + 8;
  for (const sib of [...full, ...half]) {
    if (places.has(sib.id)) continue;
    places.set(sib.id, { x: cursor, gen: 0 });
    cursor += CARD.w + CARD.gap;
  }
  cursor += 12;
