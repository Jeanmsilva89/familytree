...
function PersonChip({
  person, focused, flash, onFocus, onOpen, onRemove,
}: {
  ...
  return (
    <article className={cls} data-person-id={person.id}>
      ...
      <div className="card-actions">
        <button type="button" className="icon-btn" aria-label={`Edit / Remove ${label}`} aria-expanded={menu} onClick={() => setMenu((open) => !open)}>
          \u22ee
        </button>
        {menu ? (
  ...
  return (
    <section className="focus-family" aria-label="Family around the focus person">
      ...
      <div className="gen-stack">
        {lanes.map((lane) => (
          <LaneRow key={lane.id} lane={lane} focusIds={focusIds} onFocus={onFocus} onOpen={onOpen} onRemove={(id) => { const person = tree.people.find((p) => p.id === id); if (person) void confirmRemove(person); }} flashId={flashId} />
        ))}
      </div>
      <div className="actions focus-actions">
        <button type="button" className="btn" onClick={async () => { const name = promptName("Parent's name"); if (name) await onAddParent(focus.id, name); }}>Add parent</button>
        <button type="button" className="btn" onClick={async () => { const name = promptName("Partner's name"); if (name) await onAddPartner(focus.id, name, "partnered"); }}>Add partner</button>
        <button type="button" className="btn" onClick={async () => { const name = promptName("Child's name"); if (name) await onAddChild(childParents, name, primaryUnion?.id); }}>Add child</button>
        <button type="button" className="btn" onClick={async () => { if (!hasParents) { window.alert("Add a parent first"); return; } const name = promptName("Sibling's name"); if (!name) return; const addedId = await onAddSibling(focus.id, name); if (addedId) setFlashId(addedId); }}>Add sibling</button>
      </div>
      {peopleOpen ? (
