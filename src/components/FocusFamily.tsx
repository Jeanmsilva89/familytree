  ...
  return (
    <article className={cls} data-person-id={person.id}>
      ...
      <details className="card-more">
        <summary className="icon-btn" aria-label={`More for ${displayName(person)}`}>⋯</summary>
        <div className="card-more-menu">
          <button type="button" className="btn ghost" onClick={onOpen}>Edit</button>
...
function CoupleUnit({
  people, coupleBar, lit, focusIds, onFocus, onOpen, onRemove, flashId,
}: {
...
function LaneRow({ lane, focusIds, onFocus, onOpen, onRemove, flashId }: {
  ...
  return (
    <div className="gen-lane">
      <p className="gen-lane-title">{lane.id === "focus" ? "This generation" : lane.id[0].toUpperCase() + lane.id.slice(1)}</p>
  ...
  return (
    <section className="focus-family" aria-label="Family around the focus person">
      ...
      <div className="gen-stack">
        {lanes.map((lane) => (
          <LaneRow key={lane.id} lane={lane} focusIds={focusIds} onFocus={onFocus} onOpen={onOpen} onRemove={(id) => {
